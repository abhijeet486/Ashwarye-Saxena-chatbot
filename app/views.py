...
from .tasks import process_whatsapp_message_async
...
def handle_message(req_time):
    ...
    # incoming_timestamp = int(body['entry'][0]['changes'][0]['value']['messages'][0]['timestamp'])
    
    # # Compare the timestamps
    # if incoming_timestamp > latest_timestamp:        
    #     # Update the latest timestamp
    #     latest_timestamp = incoming_timestamp
    # else:
    #     logging.error("OLD TIMESTAMP REQUEST: ", body)
    #     return jsonify({"status": "error", "message": "OLD TIMESTAMP REQUEST"}), 400        
    ...
    try:
        if is_valid_whatsapp_message(body):
            ...
            # Dispatch Celery task instead of calling function directly
            process_whatsapp_message_async.delay(body, req_time)
